--  Standalone test suite for Prims_Algorithm (main program).
...
procedure Tests is
   ...
   function MST_Seed_Raises
     (G : Graph; Start : Vertex_Id; Buf_Last : Natural) return Boolean
   is
   ...
   begin
      if Buf_Last = 0 then
         ...
         begin
            Minimum_Spanning_Tree (G, Start, Empty_Buf, C, W);
         end;
      else
         Minimum_Spanning_Tree (G, Start, Tree (1 .. Buf_Last), C, W);
      end if;
   ...
   function MST_Seed_Raises_Bad_First
     (G : Graph; Start : Vertex_Id) return Boolean
   is
      Tree : Edge_List (2 .. Max_Vertices + 1);
      C    : Natural;
      W    : Weight_Sum;
   begin
      Minimum_Spanning_Tree (G, Start, Tree, C, W);
      pragma Unreferenced (C, W);
      return False;
   exception
      when Invalid_Argument =>
         return True;
   end MST_Seed_Raises_Bad_First;

   function MSF_Raises (G : Graph; Buf_Last : Natural) return Boolean is
      Tree : Edge_List (1 .. Positive'Max (1, Buf_Last));
      C    : Natural;
      W    : Weight_Sum;
   begin
      if Buf_Last = 0 then
         declare
            Empty_Buf : Edge_List (1 .. 0);
         begin
            Minimum_Spanning_Forest (G, Empty_Buf, C, W);
         end;
      else
         Minimum_Spanning_Forest (G, Tree (1 .. Buf_Last), C, W);
      end if;
      pragma Unreferenced (C, W);
      return False;
   exception
   ...
   function Parent_Key_Raises_Empty return Boolean is
      G      : Graph;
      Parent : Parent_Array (Vertex_Id'First .. Vertex_Id'First);
      Key    : Key_Array (Vertex_Id'First .. Vertex_Id'First);
      W      : Weight_Sum;
   begin
      Clear (G, 0);
      Minimum_Spanning_Tree (G, 1, Parent, Key, W);
      pragma Unreferenced (W);
      return False;
   exception
      when Invalid_Argument =>
         return True;
   end Parent_Key_Raises_Empty;

   function Parent_Key_Bad_Start return Boolean is
      G      : Graph;
      Parent : Parent_Array (1 .. Vertex_Id (4));
      Key    : Key_Array (1 .. Vertex_Id (4));
      W      : Weight_Sum;
   begin
      Clear (G, 3);
      Minimum_Spanning_Tree (G, 4, Parent, Key, W);
      pragma Unreferenced (W);
      return False;
   exception
      when Invalid_Argument =>
         return True;
   end Parent_Key_Bad_Start;
   ...
   procedure Agree_Forest_With_Kruskal (Label : String) is
   begin
      Minimum_Spanning_Forest (G, Tree, C, W);
      Kruskal_Reference (G, Tree_K, CK, WK);
      Check (C = CK, Label & " count agree");
      Check (W = WK, Label & " weight agree");
   end Agree_Forest_With_Kruskal;

   procedure Agree_Seed_Component
     (Start : Vertex_Id; Label : String; Exp_C : Natural; Exp_W : Weight_Sum)
   is
   begin
      Minimum_Spanning_Tree (G, Start, Tree, C, W);
      Check (C = Exp_C, Label & " seed count");
      Check (W = Exp_W, Label & " seed weight");
      Prim (G, Start, Tree_K, CK, WK);
      Check (CK = C and then WK = W, Label & " Prim alias");
   end Agree_Seed_Component;
...
begin
   ...
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (C = 0 and then W = 0, "empty MSF empty");
   Kruskal_Reference (G, Tree_K, CK, WK);
   Check (CK = 0 and then WK = 0, "empty Kruskal empty");
   Check (Parent_Key_Raises_Empty, "seeded Prim on N=0 raises");

   Clear (G, 1);
   Check (Vertex_Count (G) = 1, "single vertex count");
   Check (Edge_Count (G) = 0, "single no edges");
   Minimum_Spanning_Tree (G, 1, Parent, Key, W);
   Check (Parent (1) = 0 and then Key (1) = 0, "single Parent/Key");
   Check (W = 0, "single Total=0");
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Check (C = 0 and then W = 0, "single edge-list empty");
   Agree_Forest_With_Kruskal ("single");
   ...
   Minimum_Spanning_Tree (G, 3, Parent, Key, W);
   Check (Key (3) = 0 and then Parent (3) = 0, "edgeless seed root");
   Check (Key (1) = Infinity, "edgeless other Infinity");
   Check (W = 0, "edgeless seed weight 0");
   Agree_Forest_With_Kruskal ("edgeless");
   ...
   Agree_Forest_With_Kruskal ("textbook");
   Minimum_Spanning_Tree (G, 1, Parent, Key, WK);
   Check (Key (1) = 0 and then Parent (1) = 0, "textbook Key Start");
   Check (WK = 6, "textbook Parent/Key weight");
   Prim (G, 2, Tree, C, W);
   Check (C = 3 and then W = 6, "Prim from 2 same weight");
   ...
   Clear (G, 2);
   Add_Edge (G, 1, 2, 7);
   Agree_Seed_Component (1, "two-vert", 1, 7);
   Check (Edge_In_Tree (Tree, C, 1, 2, 7), "two-vert edge");
   Agree_Forest_With_Kruskal ("two-vert");

   Clear (G, 2);
   Add_Edge (G, 1, 2, 0);
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Check (C = 1 and then W = 0, "zero-weight edge kept");
   Agree_Forest_With_Kruskal ("zero-wt");
   ...
   Agree_Forest_With_Kruskal ("self-loops");

   Clear (G, 1);
   Add_Edge (G, 1, 1, 5);
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Check (C = 0 and then W = 0, "only self-loop → empty tree");
   Agree_Forest_With_Kruskal ("only-loop");
   ...
   Agree_Forest_With_Kruskal ("parallel");
   ...
   Agree_Forest_With_Kruskal ("parallel-tri");
   ...
   Add_Edge (G, 1, 2, 1);
   Add_Edge (G, 3, 4, 2);
   --  Seeded Prim from 1 covers only {1,2}
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Check (C = 1 and then W = 1, "seed from 1 one edge");
   Minimum_Spanning_Tree (G, 3, Tree, C, W);
   Check (C = 1 and then W = 2, "seed from 3 one edge");
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (C = 2 and then W = 3, "two components forest");
   Agree_Forest_With_Kruskal ("two-comp");
   ...
   Add_Edge (G, 4, 5, 1);
   Minimum_Spanning_Tree (G, 1, Parent, Key, W);
   Check (Key (1) = 0, "forest seed Key");
   Check (Key (4) = Infinity, "other component Infinity");
   Check (Key (6) = Infinity, "isolated Infinity");
   Check (W = 7, "component A weight 3+4");
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (C = 3, "forest count (drop 10)");
   Check (W = 8, "forest weight 3+4+1");
   Check (not Edge_In_Tree (Tree, C, 1, 3, 10), "forest drops heavy");
   Agree_Forest_With_Kruskal ("forest");

   Clear (G, 3);
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (C = 0 and then W = 0, "three isolated");
   Agree_Forest_With_Kruskal ("isolated3");
   ...
   Agree_Forest_With_Kruskal ("path5");

   Clear (G, 4);
   Add_Edge (G, 1, 2, 2);
   Add_Edge (G, 2, 3, 2);
   Add_Edge (G, 3, 4, 2);
   Minimum_Spanning_Tree (G, 4, Tree, C, W);
   Check (C = 3 and then W = 6, "equal-weight path from 4");
   Agree_Forest_With_Kruskal ("eq-path");
   ...
   Agree_Forest_With_Kruskal ("star");
   ...
   Agree_Forest_With_Kruskal ("star-chord");
   ...
   Add_Edge (G, 1, 3, 5);
   Add_Edge (G, 2, 3, 6);
   Minimum_Spanning_Tree (G, 2, Tree, C, W);
   Check (C = 2 and then W = 9, "K3 unique");
   Agree_Forest_With_Kruskal ("K3");
   ...
   Agree_Forest_With_Kruskal ("rebuild");
   ...
   Add_Edge (G, 1, 2, 1);
   Add_Edge (G, 2, 3, 2);
   --  Need Last >= N-1 = 2; Last=0 and Last=1 raise
   Check (MST_Seed_Raises (G, 1, 0), "MST seed buffer Last=0");
   Check (MST_Seed_Raises (G, 1, 1), "MST seed buffer Last=1<N-1");
   Check (MST_Seed_Raises_Bad_First (G, 1), "MST seed First/=1");
   Check (MSF_Raises (G, 1), "MSF buffer too small");
   Check (Parent_Key_Bad_Start, "Start out of range");

   ------------------------------------------------------------------
   Section ("13. Alias Prim == MST");
   ------------------------------------------------------------------
   Clear (G, 4);
   Add_Edge (G, 1, 2, 2);
   Add_Edge (G, 2, 3, 3);
   ...
   Add_Edge (G, 1, 3, 5);
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Prim (G, 1, Tree_K, CK, WK);
   Check (C = CK and then W = WK, "alias totals");
   Check (C = 3, "alias count 3");
   Prim (G, 1, Parent, Key, WK);
   Check (WK = W, "alias Parent/Key weight");
   ...
   Agree_Forest_With_Kruskal ("grid");
   ...
   for N in 2 .. 8 loop
      ...
      Agree_Forest_With_Kruskal ("path-N" & Integer'Image (N));
   end loop;
   ...
   for N in 3 .. 7 loop
      ...
      Agree_Forest_With_Kruskal ("cyc-N" & Integer'Image (N));
   end loop;
   ...
   ------------------------------------------------------------------
   Section ("18. Cut-property stress");
   ------------------------------------------------------------------
   Clear (G, 5);
   Add_Edge (G, 1, 2, 100);
   Add_Edge (G, 2, 3, 90);
   ...
   Agree_Forest_With_Kruskal ("stress");
   ...
   Add_Edge (G, 7, 7, 99);
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Check (C = 2 and then W = 3, "mix seed A only");
   Minimum_Spanning_Forest (G, Tree, C, W);
   ...
   Agree_Forest_With_Kruskal ("max-span");
   ...
   Agree_Forest_With_Kruskal ("50par");
   ...
   Agree_Forest_With_Kruskal ("tree6");
   ...
   Agree_Forest_With_Kruskal ("path7h");
   ...
   Agree_Forest_With_Kruskal ("all-zero");
   ...
   Agree_Forest_With_Kruskal ("zero-ties");
   ...
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (W = WK and then C = CK, "Prim MSF matches Kruskal");
   ...
   Add_Edge (G, 4, 6, 6);
   Minimum_Spanning_Tree (G, 2, Tree, C, W);
   Check (C = 2 and then W = 3, "seed B side A only");
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (C = 4, "2comp internal count");
   Check (W = 1 + 2 + 4 + 5, "2comp internal weight");
   Agree_Forest_With_Kruskal ("2comp-int");
   ...
   declare
      ...
      We    : Weight_Sum;
   begin
      Minimum_Spanning_Tree (G, 1, Exact, Ce, We);
      Check (Ce = 2 and then We = 3, "exact buffer OK");
      Minimum_Spanning_Forest (G, Exact, Ce, We);
      Check (Ce = 2 and then We = 3, "exact MSF buffer OK");
   end;
   ...
   Agree_Forest_With_Kruskal ("rev-ep");
   ...
   Agree_Forest_With_Kruskal ("star8");
   ...
   Agree_Forest_With_Kruskal ("hub-path");
   ...
   Agree_Forest_With_Kruskal ("classic5");
   ...
   Agree_Forest_With_Kruskal ("almost-bi");
   ...
   Minimum_Spanning_Tree (G, 1, Tree, C, W);
   Check (C = 1 and then W = 1, "seed one of five");
   Minimum_Spanning_Forest (G, Tree, C, W);
   Check (C = 5 and then W = 15, "five disjoint edges");
   Agree_Forest_With_Kruskal ("five-disj");

   ------------------------------------------------------------------
   Section ("30. Parent/Key Infinity + final cross-checks");
   ------------------------------------------------------------------
   Clear (G, 6);
   Add_Edge (G, 1, 2, 2);
   Add_Edge (G, 2, 3, 2);
   ...
   Agree_Forest_With_Kruskal ("bridged-tri");

   Minimum_Spanning_Tree (G, 1, Parent, Key, W);
   Check (Key (1) = 0, "bridged Key Start");
   Check (W = 9, "bridged connected weight");
   Check (Key (6) /= Infinity, "bridged all reachable");

   Prim (G, 1, Tree, C, W);
   Kruskal_Reference (G, Tree_K, CK, WK);
   Check (C = CK and then W = WK, "final Prim=Kruskal");
   ...
   --  Infinity sentinel exposed
   Clear (G, 2);
   Minimum_Spanning_Tree (G, 1, Parent, Key, W);
   Check (Key (2) = Infinity, "Infinity sentinel on unreachable");
   declare
      function KV (X : Key_Value) return Key_Value is (X);
   begin
      Check (KV (Infinity) = Key_Value'Last, "Infinity = Key_Value'Last");
   end;
