...
package body Minimum_Depth_Of_Binary_Tree is
   ...
   function Min_Depth (T : Tree; Root : Index) return Depth is
      Nodes : Node_Stack := (others => 0); Levels : Depth_Stack := (others => 0);
      Top : Stack_Position; Best : Depth := Depth'Last;
   begin
      ...
      for Step in 1 .. 256 loop
         ...
         if Top > 0 then
            declare N : constant Index := Nodes (Top); D : constant Depth := Levels (Top);
            begin
               Top := Top - 1;
               if T.Used (N) then
                  if T.Lefts (N) = 0 and then T.Rights (N) = 0 then
                     if D < Best then Best := D; end if;
                  elsif D < Best and then D < Depth'Last then
                     if T.Lefts (N) /= 0 and then Top < Stack_Position'Last then
                        Top := Top + 1; Nodes (Top) := T.Lefts (N); Levels (Top) := D + 1;
                     end if;
                     if T.Rights (N) /= 0 and then Top < Stack_Position'Last then
                        Top := Top + 1; Nodes (Top) := T.Rights (N); Levels (Top) := D + 1;
                     end if;
                  end if;
               end if;
            end;
...
end Minimum_Depth_Of_Binary_Tree;
