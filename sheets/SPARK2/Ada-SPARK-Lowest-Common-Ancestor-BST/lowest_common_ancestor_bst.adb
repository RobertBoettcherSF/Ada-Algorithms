...
package body Lowest_Common_Ancestor_BST is
   ...
   function Lowest_Common_Ancestor (T : Tree; Root : Index; A, B : Value) return Index is
      Current : Index := Root;
      Answer : Index := 0;
   begin
      for Step in 1 .. 31 loop
         if Current = 0 or else not T.Used (Current) then exit; end if;
         if A < T.Values (Current) and then B < T.Values (Current) then
            Current := T.Lefts (Current);
         elsif A > T.Values (Current) and then B > T.Values (Current) then
...
end Lowest_Common_Ancestor_BST;
