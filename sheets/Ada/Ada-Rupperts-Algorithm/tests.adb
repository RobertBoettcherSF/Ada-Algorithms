--  Standalone test suite for Rupperts_Algorithm (main program).

pragma Ada_2022;

with Ada.Command_Line;
with Ada.Text_IO;
with Rupperts_Algorithm; use Rupperts_Algorithm;
...
begin
   Ada.Text_IO.Put_Line ("Rupperts_Algorithm tests");
   ...
   ------------------------------------------------------------------
   declare
      --  Right isosceles: angles 45-45-90
      A : constant Point := P (0.0, 0.0);
      B : constant Point := P (1.0, 0.0);
      C : constant Point := P (0.0, 1.0);
      Min_A : Real;
   ...
   ------------------------------------------------------------------
   Section ("4. Encroachment (diametral circle)");
   ------------------------------------------------------------------
   declare
      A : constant Point := P (0.0, 0.0);
      B : constant Point := P (2.0, 0.0);
      --  Midpoint of AB is (1,0); diametral radius = 1.
      Inside  : constant Point := P (1.0, 0.5);   -- dist to mid = 0.5 < 1
      Outside : constant Point := P (1.0, 1.5);   -- dist = 1.5 > 1
      On_Circ : constant Point := P (1.0, 1.0);   -- dist = 1 (not strict)
   begin
   ...
   ------------------------------------------------------------------
   Section ("9. Refine: good triangle maintains quality");
   ------------------------------------------------------------------
   declare
      Pts : constant Point_Array :=
        [P (0.0, 0.0), P (1.0, 0.0), P (0.5, 0.86602540378)];
      --  Near-equilateral; min angle ~60
      Before : constant Triangulation := Triangulate (Pts);
      Ang0 : constant Real := Mesh_Min_Angle_Degrees (Pts, Before);
      Res : constant Refine_Result := Refine (Pts, R (20.0), 8);
   begin
      Check (Ang0 > R (50.0), "equilateral input already good");
      Check (Res.Steiner_Inserted = 0, "equilateral needs 0 Steiner @20");
      Check (Result_Point_Count (Res) = 3, "equilateral keeps 3 points");
      Check (Triangle_Count_Of (Res.Mesh) = 1, "equilateral still 1 tri");
      Check (Res.Min_Angle_Achieved >= R (20.0) - R (1.0E-3),
             "equilateral min angle >= bound");
      Check (Res.Steiner_Inserted <= 8, "Steiner count within budget");
   end;
   ...
   declare
      ...
      Before : constant Triangulation := Triangulate (Pts);
      Ang0 : constant Real := Mesh_Min_Angle_Degrees (Pts, Before);
      Res : constant Refine_Result := Refine (Pts, R (20.0), 16);
   ...
   ------------------------------------------------------------------
   declare
      --  Thin kite / near-collinear fourth point → skinny triangles
      Pts : constant Point_Array :=
        [P (0.0, 0.0), P (4.0, 0.0), P (0.0, 3.0), P (0.05, 0.05)];
      Before : constant Triangulation := Triangulate (Pts);
      Ang0 : constant Real := Mesh_Min_Angle_Degrees (Pts, Before);
      Skin0 : constant Natural := Count_Skinny (Pts, Before, R (20.0));
      Res : constant Refine_Result := Refine (Pts, R (20.0), 24);
      Ang1 : constant Real := Res.Min_Angle_Achieved;
   begin
   ...
   declare
      ...
      Res : constant Refine_Result := Refine (Pts, R (25.0), 0);
   begin
   ...
   ------------------------------------------------------------------
   Section ("13. Refine_With_Segments (PSLG sketch)");
   ------------------------------------------------------------------
   declare
      Pts : constant Point_Array :=
      ...
      Res : constant Refine_Result :=
        Refine_With_Segments (Pts, Segs, R (15.0), 20);
   begin
   ...
   begin
      declare
         Res : Refine_Result;
      begin
         Res := Refine_With_Segments (Pts, Bad_Seg, R (20.0), 4);
         pragma Unreferenced (Res);
      exception
         when Invalid_Argument =>
            Raised := True;
   ...
   declare
      ...
      Res : constant Refine_Result := Refine (Pts, R (10.0), 4);
      Tri : Triangle;
   begin
