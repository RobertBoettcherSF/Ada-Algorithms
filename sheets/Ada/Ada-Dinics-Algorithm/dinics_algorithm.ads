--  Dinics_Algorithm — Ada 2023 educational package for Dinic's (Dinitz's)
--  algorithm: maximum s–t flow via repeated level-graph (BFS) phases and
--  blocking flows (DFS along level edges). Integer capacities on a directed
--  flow network. After a successful Max_Flow call the residual graph yields
--  a min-cut partition (source-side reachability) and per-edge flows.
...
--  Sibling sheets (README only — do not `with`): Edmonds–Karp, Ford–Fulkerson,
...
is
   ...
   ---------------------------------------------------------------------------
   -- Algorithm sketch (Dinic)
   ---------------------------------------------------------------------------
   --  Maintain residual capacity c_f(u,v). While a BFS from Source reaches
   --  Sink in the residual graph, build the level graph: Level(v) is the
   --  hop distance from Source, and level edges satisfy
   --  Level(v) = Level(u) + 1. Then push a blocking flow by DFS along level
   --  edges (current-edge pointers) until no Source↝Sink level path remains.
   --  Each phase strictly increases the Source–Sink distance; there are
   --  O(V) phases, each O(VE), hence O(V²E). When no residual Source↝Sink
   --  path remains, residual reachability from Source forms the source side
   --  S of a minimum s–t cut; |f| equals the cut capacity (max-flow min-cut).
   --  Contrast (README only): Edmonds–Karp (BFS one path per phase), classic
   --  Ford–Fulkerson (unspecified / DFS paths), Push–relabel (local height).

   function Max_Flow
     (G : in out Graph; Source, Sink : Vertex_Id) return Flow_Value
     with Global => null;
   --  Dinic: reset flows, then alternate BFS level graphs with DFS blocking
   --  flows until Sink is unreachable. Returns the maximum Source→Sink flow.
