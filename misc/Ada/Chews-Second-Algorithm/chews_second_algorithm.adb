--  Chews_Second_Algorithm body — Delaunay refinement (educational Float).

pragma Ada_2022;

with Ada.Numerics.Elementary_Functions;

package body Chews_Second_Algorithm
  with SPARK_Mode => Off
is
   ...
   ---------------------------------------------------------------------------
   -- Encroachment / Chew opposite-side
   ---------------------------------------------------------------------------
   ...
   is
      O_T : constant Real := Orient2D (Seg_A, Seg_B, Tri_Rep);
      O_C : constant Real := Orient2D (Seg_A, Seg_B, Circ);
   begin
      --  Strict opposite sides (product negative).
      return O_T * O_C < -Epsilon;
   end Circumcenter_Opposite_Side;
   ...
   function Triangle_Centroid
     (A, B, C : Point) return Point
   is
   begin
      return (X => (A.X + B.X + C.X) / 3.0,
              Y => (A.Y + B.Y + C.Y) / 3.0);
   end Triangle_Centroid;

   --  Find a constrained segment that rejects circumcenter Candidate of
   --  skinny triangle with representative Tri_Rep (Chew opposite-side or
   --  diametral encroachment).
   function Find_Rejecting_Segment
     (Candidate : Point;
      Tri_Rep   : Point;
      Work      : Point_Array;
      Segs      : Segment_Array;
   ...
   begin
      ...
      for Si in 1 .. Seg_N loop
         declare
            ...
            SA : constant Point := Work (S.A);
            SB : constant Point := Work (S.B);
         begin
            if Circumcenter_Opposite_Side (SA, SB, Tri_Rep, Candidate)
              or else Encroaches_Segment (SA, SB, Candidate)
            then
               Enc_Idx := Si;
               return True;
            end if;
      ...
      return False;
   end Find_Rejecting_Segment;

   type Boolean_Array is array (Point_Index range <>) of Boolean;

   procedure Split_Segment_Chew
     (Work      : in out Point_Array;
      N         : in out Point_Count;
      Is_Circum : in out Boolean_Array;
   ...
   is
      ...
      SB : constant Point := Work (S.B);
      Mid : Point;
      Removable : Boolean_Array (1 .. Max_Points) := [others => False];
      New_Work : Point_Array (1 .. Max_Points + 3);
      New_Is   : Boolean_Array (1 .. Max_Points) := [others => False];
      Map      : array (1 .. Max_Points) of Point_Index := [others => 1];
      New_N    : Point_Count := 0;
      Cnt_Rem  : Natural := 0;
   begin
      --  Mark prior circumcenter Steiner points inside diametral ball of
      --  the original segment (Chew cleanup), excluding endpoints.
      for I in 1 .. Point_Index (N) loop
         if Is_Circum (I)
           and then I /= S.A
           and then I /= S.B
           and then Encroaches_Segment (SA, SB, Work (I))
         then
            Removable (I) := True;
            Cnt_Rem := Cnt_Rem + 1;
         end if;
      ...
      if Cnt_Rem > 0 then
         for I in 1 .. Point_Index (N) loop
            if not Removable (I) then
               New_N := New_N + 1;
               New_Work (Point_Index (New_N)) := Work (I);
               New_Is (Point_Index (New_N)) := Is_Circum (I);
               Map (I) := Point_Index (New_N);
            end if;
         end loop;
         for Si in 1 .. Seg_N loop
            Segs (Si).A := Map (Segs (Si).A);
            Segs (Si).B := Map (Segs (Si).B);
         end loop;
         for I in 1 .. Point_Index (New_N) loop
            Work (I) := New_Work (I);
            Is_Circum (I) := New_Is (I);
         end loop;
         N := New_N;
      end if;

      --  Midpoint of (possibly remapped) segment at Seg_Idx.
      declare
         S2 : constant Segment := Segs (Seg_Idx);
      begin
         if N = Max_Points then
            raise Capacity_Exceeded with "Max_Points reached during split";
         end if;
         Mid :=
           (X => (Work (S2.A).X + Work (S2.B).X) / 2.0,
            Y => (Work (S2.A).Y + Work (S2.B).Y) / 2.0);
         N := N + 1;
         New_Idx := Point_Index (N);
         Work (New_Idx) := Mid;
         Is_Circum (New_Idx) := False;  --  midpoint, not circumcenter
         Segs (Seg_Idx) := (A => S2.A, B => New_Idx);
         if Seg_N = Max_Segments then
            raise Capacity_Exceeded with "Max_Segments reached during split";
         end if;
         Seg_N := Seg_N + 1;
         Segs (Seg_N) := (A => New_Idx, B => S2.B);
      end;
   end Split_Segment_Chew;
   ...
   is
      ...
      Work : Point_Array (1 .. Max_Points + 3);
      Is_Circum : Boolean_Array (1 .. Max_Points) := [others => False];
      N : Point_Count;
      ...
      Box : Bounding_Box;
      Margin : Real;
      Found_Skinny : Boolean;
      Skinny_Tri : Triangle;
      Enc_Idx : Segment_Index;
      New_Idx : Point_Index;
      Candidate, Tri_Rep : Point;
      Progress : Boolean;
   begin
         ...
         then
            Candidate := Circumcenter
              (Work (Skinny_Tri.A),
               Work (Skinny_Tri.B),
               Work (Skinny_Tri.C));
            Tri_Rep := Triangle_Centroid
              (Work (Skinny_Tri.A),
               Work (Skinny_Tri.B),
               Work (Skinny_Tri.C));

            if Seg_N > 0
              and then Find_Rejecting_Segment
                (Candidate, Tri_Rep, Work, Segs, Seg_N, Enc_Idx)
            then
               --  Chew: split rejected segment; clean diametral Steiner.
               Split_Segment_Chew
                 (Work, N, Is_Circum, Segs, Seg_N, Enc_Idx, New_Idx);
               Steiner := Steiner + 1;
               Mesh := Triangulate (Slice_Points (Work, N));
               Progress := True;
