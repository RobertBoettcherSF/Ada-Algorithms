...
package body Sum_Of_Left_Leaves is
   ...
   function Sum_Left_Leaves (T : Tree; Root : Index) return Sum is
      Nodes : Node_Stack := (others => 0); Left_Flags : Flag_Stack := (others => False);
   ...
   begin
      ...
      for Step in 1 .. 256 loop
         ...
         if Top > 0 then
            declare N : constant Index := Nodes (Top); Is_Left : constant Boolean := Left_Flags (Top);
            begin
               Top := Top - 1;
               if T.Used (N) then
                  if Is_Left and then T.Lefts (N) = 0 and then T.Rights (N) = 0 then
                     if T.Values (N) > 0 then
                     ...
                     if T.Rights (N) /= 0 and then Top < Stack_Position'Last then
                        Top := Top + 1; Nodes (Top) := T.Rights (N); Left_Flags (Top) := False;
                     end if;
                     if T.Lefts (N) /= 0 and then Top < Stack_Position'Last then
                        Top := Top + 1; Nodes (Top) := T.Lefts (N); Left_Flags (Top) := True;
                     end if;
                  end if;
               end if;
            end;
