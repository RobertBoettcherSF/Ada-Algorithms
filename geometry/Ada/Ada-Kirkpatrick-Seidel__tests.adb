--  Standalone test suite for Kirkpatrick_Seidel (main program).
...
with Ada.Text_IO;
with Kirkpatrick_Seidel; use Kirkpatrick_Seidel;
...
procedure Tests is
   ...
   function Raised_Invalid_Bridge
     (L, R : Point_Set) return Boolean
   is
   begin
      declare
         B : constant Bridge_Edge := Upper_Bridge (L, R);
      begin
...
begin
   Ada.Text_IO.Put_Line ("Kirkpatrick_Seidel tests");
   ...
   ------------------------------------------------------------------
   Section ("2. Invalid_Argument: empty / oversized / empty bridge");
   ------------------------------------------------------------------
   ...
   declare
      ...
      L1  : constant Point_Set := [P (0.0, 0.0)];
   begin
      Check (Raised_Invalid_Area (Two), "Signed_Area n=2 raises");
      Check (Raised_Invalid_Bridge (Empty_Set, L1), "Bridge empty L raises");
      Check (Raised_Invalid_Bridge (L1, Empty_Set), "Bridge empty R raises");
   end;

   ------------------------------------------------------------------
   Section ("3. Upper_Bridge / Lower_Bridge on square halves");
   ------------------------------------------------------------------
   declare
      Left  : constant Point_Set := [P (0.0, 0.0), P (0.0, 1.0)];
      Right : constant Point_Set := [P (1.0, 0.0), P (1.0, 1.0)];
      Ub : constant Bridge_Edge := Upper_Bridge (Left, Right);
      Lb : constant Bridge_Edge := Lower_Bridge (Left, Right);
   begin
      Check (Near_Point (Ub.Left, P (0.0, 1.0)), "upper bridge left top");
      Check (Near_Point (Ub.Right, P (1.0, 1.0)), "upper bridge right top");
      Check (Near_Point (Lb.Left, P (0.0, 0.0)), "lower bridge left bottom");
      Check (Near_Point (Lb.Right, P (1.0, 0.0)), "lower bridge right bottom");
   end;

   ------------------------------------------------------------------
   Section ("4. Single point / two points / duplicates");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("7. Interior points ignored");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("8. Points already in convex position");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("9. Regular-ish octagon + jitter interior");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("10. Collinear educational cases");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("11. Diamond / rotated square");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("12. Random-ish classroom cloud vs Andrew");
   ------------------------------------------------------------------
   declare
      Cloud : constant Point_Set :=
   ...
   ------------------------------------------------------------------
   Section ("13. Max_Points capacity / shuffled square");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("14. Signed_Area / Is_CCW helpers");
   ------------------------------------------------------------------
   declare
   ...
   ------------------------------------------------------------------
   Section ("15. Bridge on triangle split / more bridges");
   ------------------------------------------------------------------
   declare
      --  Split triangle: left {(0,0),(1,3)} right {(4,0)}
      L : constant Point_Set := [P (0.0, 0.0), P (1.0, 3.0)];
      R : constant Point_Set := [P (4.0, 0.0)];
      Ub : constant Bridge_Edge := Upper_Bridge (L, R);
      Lb : constant Bridge_Edge := Lower_Bridge (L, R);
      --  Wider cloud split
      L2 : constant Point_Set :=
        [P (0.0, 0.0), P (0.0, 4.0), P (1.0, 1.0)];
      R2 : constant Point_Set :=
        [P (5.0, 0.0), P (5.0, 4.0), P (4.0, 2.0)];
      Ub2 : constant Bridge_Edge := Upper_Bridge (L2, R2);
      Lb2 : constant Bridge_Edge := Lower_Bridge (L2, R2);
   begin
      Check (Near_Point (Ub.Left, P (1.0, 3.0)), "tri upper left peak");
      Check (Near_Point (Ub.Right, P (4.0, 0.0)), "tri upper right");
      Check (Near_Point (Lb.Left, P (0.0, 0.0)), "tri lower left origin");
      Check (Near_Point (Lb.Right, P (4.0, 0.0)), "tri lower right");
      Check (Near_Point (Ub2.Left, P (0.0, 4.0)), "cloud upper L");
      Check (Near_Point (Ub2.Right, P (5.0, 4.0)), "cloud upper R");
      Check (Near_Point (Lb2.Left, P (0.0, 0.0)), "cloud lower L");
      Check (Near_Point (Lb2.Right, P (5.0, 0.0)), "cloud lower R");
   end;
