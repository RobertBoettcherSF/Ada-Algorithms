--  Standalone test suite for Patience_Sorting (main program).
...
procedure Tests is
   ...
   --  indistinguishable, so Same still holds for patience sort vs reference.
...
begin
   ...
   --  Worked README example: deal [4,3,9,1,8] → merge to sorted.
   Expect_Sorted ([4, 3, 9, 1, 8], "readme deal example");
   ...
   Expect_Sorted (Random_Array (256, 0, 99), "random n=256");
   Expect_Sorted (Random_Array (128, -20, 20), "random n=128");
   ...
   begin
      Check (Sort_Raises (Big), "Max_N+1 raises Invalid_Argument");
   end;
   declare
      Exact : Element_Array (1 .. Max_N);
   begin
      --  Fill with a reverse pattern so we do not create n piles of height
      --  1 (already-sorted would make merge O(n²) and be slow at Max_N).
      --  Reverse yields a single pile → O(n) deal+merge.
      for I in Exact'Range loop
         Exact (I) := Max_N + 1 - I;
      end loop;
      Sort (Exact);
      Check (Is_Sorted (Exact), "exact Max_N reverse sorts");
      Check (not Sort_Raises (Exact), "exact Max_N does not raise");
   end;
   declare
      Tiny : constant Element_Array := [1, 2];
   begin
      Check (not Sort_Raises (Tiny), "small array does not raise");
   end;

   ---------------------------------------------------------------------
   Section ("9. Length patterns (sorted / reverse / mixed)");
   ---------------------------------------------------------------------
   declare
   ...
   begin
      ...
      Expect_Sorted (A, "sorted n=64 (many piles)");
   end;
   declare
      A : Element_Array (1 .. 8);
   begin
      ...
      Expect_Sorted (A, "reverse n=8 (one pile)");
   end;
   ...
   Expect_Sorted ([4, 3, 2, 1, 4, 3, 2, 1], "two reverse runs");
   Expect_Sorted ([1, 3, 5, 7, 9, 2, 4, 6, 8], "odds then evens");
