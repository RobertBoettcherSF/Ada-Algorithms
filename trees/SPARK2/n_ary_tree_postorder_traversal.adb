...
package body N_Ary_Tree_Postorder_Traversal is
   ...
   function Postorder (T : Tree; Root : Index) return Visit_Result is
      type Node_Array is array (Node_Index) of Index;
      Stack : Node_Array := (others => 0);
      Temp : Visit_Result := (Values => (others => 0), Length => 0);
   ...
   begin
      ...
      for Step in 1 .. 16 loop
         ...
         if Current /= 0 and then T.Used (Current) then
            if Temp.Length < 16 then
               ...
               Temp.Values (Temp.Length) := T.Values (Current);
            end if;
      ...
      for K in reverse Node_Index loop
         if K <= Temp.Length and then Result.Length < 16 then
            Result.Length := Result.Length + 1;
            Result.Values (Result.Length) := Temp.Values (K);
         end if;
      end loop;
      return Result;
   end Postorder;
end N_Ary_Tree_Postorder_Traversal;
